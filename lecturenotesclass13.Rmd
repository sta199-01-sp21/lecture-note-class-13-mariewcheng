...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-mariewcheng/manhattan.csv")
```
...
No, I get a different number each time.
...
- Parameter of interest: Mean rent price
...
get_ci(boot_median, level = 0.95)
  
```

The distribution of median rent is unimodal but hsa a right shift. The spread is from 2162-2875.
...
- Population: Mice not on treatments
- Parameter of interest: Proportion
- Sample: Mice not on treatments
- Sample size: 261
...
We are 95% confident that the true proportion of mice that died is between 69.0% to 79.7%
...
We are 90% confident that the true proportion of mice that died is between 55.2% to 65.1%
